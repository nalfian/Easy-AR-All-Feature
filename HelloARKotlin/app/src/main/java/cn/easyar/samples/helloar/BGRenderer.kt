...
class BGRenderer {
    ...
    private fun retrieveFrame(format: Int, width: Int, height: Int, buffer: ByteBuffer, retrieve_count: Int) {
        if (retrieve_count == 0) {
            ...
            when (background_shader_) {
                BGRenderer.FrameShader.RGB -> when (format) {
                ...
                BGRenderer.FrameShader.YUV -> if (format == PixelFormat.YUV_NV21 || format == PixelFormat.YUV_NV12 ||
