...
private:
    void retrieveFrame(easyar::PixelFormat format, int width, int height, void * bufferData, int retrieve_count);
    ...
    FrameShader background_shader_;
...
GLES20Renderer::GLES20Renderer()
{
    ...
    current_format_ = PixelFormat_Unknown;
    current_image_size_ = Vec2I(0, 0);
}
    ...
    do
    {
        ...
        switch (background_shader_)
        {
            ...
            case FrameShader_YUV:
                ...
                glBindTexture(GL_TEXTURE_2D, background_texture_id_);
                retrieveFrame(format, width, height, bufferData, 0);
                if (format == PixelFormat_YUV_NV21 || format == PixelFormat_YUV_NV12)
                {
                    glActiveTexture(GL_TEXTURE1);
...
void GLES20Renderer::render(Matrix44F imageProjection)
{
    ...
    do {
        ...
        easyar::Vec4F v0 = mul(imageProjection, Vec4F(vertices[0], vertices[1], vertices[2], 1));
        easyar::Vec4F v1 = mul(imageProjection, Vec4F(vertices[3], vertices[4], vertices[5], 1));
        easyar::Vec4F v2 = mul(imageProjection, Vec4F(vertices[6], vertices[7], vertices[8], 1));
        easyar::Vec4F v3 = mul(imageProjection, Vec4F(vertices[9], vertices[10], vertices[11], 1));
        memcpy(&vertices[0], v0.data, 3 * sizeof(float));
        memcpy(&vertices[3], v1.data, 3 * sizeof(float));
        memcpy(&vertices[6], v2.data, 3 * sizeof(float));
        memcpy(&vertices[9], v3.data, 3 * sizeof(float));
        ...
        case FrameShader_YUV:
            glActiveTexture(GL_TEXTURE0);
            glBindTexture(GL_TEXTURE_2D, background_texture_id_);
            if (current_format_ == PixelFormat_YUV_NV21 || current_format_ == PixelFormat_YUV_NV12) {
                glActiveTexture(GL_TEXTURE1);
                glBindTexture(GL_TEXTURE_2D, background_texture_uv_id_);
            } else {
                glActiveTexture(GL_TEXTURE1);
...
void GLES20Renderer::retrieveFrame(easyar::PixelFormat format, int width, int height, void * bufferData, int retrieve_count)
{
    if (retrieve_count == 0) {
        if (width & 0x3) {
            if (width & 0x1)
        ...
        case FrameShader_RGB:
            ...
            break;
        case FrameShader_YUV:
            if (format == PixelFormat_YUV_NV21 || format == PixelFormat_YUV_NV12 || format == PixelFormat_YUV_I420 || format == PixelFormat_YUV_YV12) {
                glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, width, height, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, bufferData);
            } else {
        ...
        } else if (format == PixelFormat_YUV_I420) {
            type = GL_LUMINANCE;
        ...
        } else if (format == PixelFormat_YUV_YV12) {
            type = GL_LUMINANCE;
            if (retrieve_count == 1) { //U
                data = (unsigned char *)bufferData + width * height * 5 / 4;
            } else if (retrieve_count == 2) { //V
...
void Renderer::render(easyar::Matrix44F imageProjection)
{
